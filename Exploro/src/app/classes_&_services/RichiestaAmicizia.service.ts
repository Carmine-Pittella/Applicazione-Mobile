...
export class RichiestaAmiciziaService {
  ...
  accettaRichiestaDiAmicizia(r: RichiestaAmicizia) {
    let index = this.getIndexRichiestaById(r.id)
    this.richiesteList[index].conferma = true
    this.geocacherSrv.aggiungiAmicizia(r.Gchiede, r.Griceve);
  }

  declinaRichiestaDiAmicizia(r: number) {
    this.richiesteList = this.richiesteList.filter(u => { u.id !== r });
    // penso che in questo caso la richiesta andrebbe rimossa dalla lista, così nel caso la si rimanda
  }
